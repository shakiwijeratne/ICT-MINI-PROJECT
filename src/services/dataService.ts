...
export async function getDiaries(
  ...
  if (isFirebaseConfigured && db) {
    try {
      ...
      console.error(
        'Error fetching diaries from Firestore:',
        error
      );
      return [];
    }
...
export async function createInternship(
  ...
  // Prevent the same student from being assigned
  // to more than one internship.
  ...
  if (existingInternships.length > 0) {
    throw new Error(
      'This student is already assigned to an internship.'
    );
  }
